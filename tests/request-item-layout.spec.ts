...
test.describe('Request Item Layout Test', () => {
  ...
  test('should display request type before request name without icon', async ({
    page,
  }) => {
  ...
  test('should maintain inline editing functionality with method badge', async ({
    page,
  }) => {
    const collectionItems = page.locator(
      '[data-testid*="collection"], .collection-item'
    ...
    if (collectionCount > 0) {
      ...
      if (requestCount > 0) {
        ...
        const requestName = firstRequest
        ...
        if (await requestName.isVisible()) {
          ...
          if (await inputField.isVisible()) {
            ...
            const methodBadge = firstRequest
              .locator('[class*="badge"]')
              .first();
            if (await methodBadge.isVisible()) {
  ...
  test('should show different HTTP methods with appropriate colors', async ({
    page,
  }) => {
    const collectionItems = page.locator(
      '[data-testid*="collection"], .collection-item'
  ...
  test('should not display request icon anywhere in the request item', async ({
    page,
  }) => {
    const collectionItems = page.locator(
      '[data-testid*="collection"], .collection-item'
