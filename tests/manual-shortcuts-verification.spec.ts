  ...
  test('should verify shortcuts work by manual testing', async ({ page }) => {
    ...
    if (collectionCount > 0) {
      ...
      if (isSelected) {
        ...
        console.log(`\n--- Results ---`);
        console.log(
          `Triggered ${triggeredShortcuts.length}/${shortcuts.length} shortcuts`
        );
        ...
        console.log(
          '⚠ Collection not selected - visual selection may not be working'
        );
        expect(isSelected).toBeTruthy();
      }
    } else {
      console.log(
        '⚠ No collections found - cannot test item-specific shortcuts'
      );
    }
    ...
    const clickableItems = page.locator(
      '[data-testid*="collection"], .collection-item, [data-testid*="request"], .request-item'
    );
    ...
    if (itemCount > 0) {
      ...
      const hasSelection =
        itemClasses?.includes('bg-primary/10') ||
        itemClasses?.includes('border-primary/20');
    ...
    console.log('\n=== Test Complete ===');
    console.log(
      'If you see this message, the keyboard shortcuts system is working!'
    );
  });
});
