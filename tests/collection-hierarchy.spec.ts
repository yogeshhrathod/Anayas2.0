...
test.describe('Collection Hierarchy Tests', () => {
  ...
  test.describe('Collection Display and Structure', () => {
    test('should display collections in sidebar hierarchy', async ({
      page,
    }) => {
      ...
      if (collectionCount > 0) {
        ...
        await expect(firstCollection.locator('text=★')).toBeVisible({
          timeout: 5000,
        });
  ...
  test.describe('Expand/Collapse Functionality', () => {
    ...
    test('should toggle chevron icon when expanding/collapsing', async ({
      page,
    }) => {
      const collectionItems = page.locator(
        '[data-testid*="collection"], .collection-item'
      ...
      if (collectionCount > 0) {
        ...
        const chevronRight = expandButton.locator(
          'svg[data-lucide="chevron-right"]'
        );
        const chevronDown = expandButton.locator(
  ...
  test.describe('Request Display and Interaction', () => {
    test('should display requests under collections when expanded', async ({
      page,
    }) => {
      const collectionItems = page.locator(
        '[data-testid*="collection"], .collection-item'
  ...
  test.describe('Context Menu and Actions', () => {
    test('should show context menu on right-click collection', async ({
      ...
      if (collectionCount > 0) {
        ...
        const contextMenu = page.locator(
          '[role="menu"], .context-menu, [data-testid*="menu"]'
        );
    ...
    test('should show context menu on right-click request', async ({
      page,
    }) => {
      const collectionItems = page.locator(
        '[data-testid*="collection"], .collection-item'
    ...
    test('should support dragging collections', async ({ page }) => {
      ...
      if (collectionCount > 1) {
        ...
        const secondDraggable =
          await secondCollection.getAttribute('draggable');

        // At least one should be draggable
        expect(
          firstDraggable === 'true' || secondDraggable === 'true'
        ).toBeTruthy();
      }
    });
  ...
  test.describe('Collection Creation and Management', () => {
      ...
      if (await createButton.isVisible()) {
        ...
        if (await nameInput.isVisible()) {
          ...
          if (await saveButton.isVisible()) {
            ...
            await expect(
              page.locator('text=Test Collection Hierarchy')
            ).toBeVisible();

            // Verify it appears in hierarchy without "0" suffix
            const newCollection = page
              .locator('text=Test Collection Hierarchy')
              .first();
    ...
    test('should create new request in collection', async ({ page }) => {
      ...
      if (collectionCount > 0) {
        ...
        const addRequestOption = page
          .locator('text=Add Request, text=New Request')
          .first();
  ...
  test.describe('Error Handling and Edge Cases', () => {
    ...
    test('should maintain hierarchy state after page refresh', async ({
      page,
    }) => {
      const collectionItems = page.locator(
        '[data-testid*="collection"], .collection-item'
      ...
      if (collectionCount > 0) {
        ...
        const refreshedCollections = page.locator(
          '[data-testid*="collection"], .collection-item'
        );
