...
test.describe('UI Persistence Check - What User Actually Sees', () => {
  ...
  test('should check if collections and requests appear in UI after creation', async ({ page }) => {
    ...
    await page.screenshot({ path: 'initial-collections-page.png', fullPage: true });
    
    // Count any existing collections
    const allText = await page.textContent('body');
    console.log('Collections page content:', allText?.substring(0, 500) + '...');
    ...
    if (await newCollectionBtn.count() > 0) {
      ...
      if (await nameInput.count() > 0) {
        ...
        if (await saveBtn.count() > 0) {
          ...
          await page.screenshot({ path: 'after-collection-creation.png', fullPage: true });
        } else {
            ...
            if (await nameInput.count() > 0) {
              ...
              if (await confirmBtn.count() > 0) {
                ...
                if (sidebarText?.includes('UI Test Request')) {
                  ...
                  console.log('❌ Request does NOT appear in sidebar after creation');
                }
                
                // Take screenshot after request creation
                await page.screenshot({ path: 'after-request-creation.png', fullPage: true });
              } else {
    ...
    await page.screenshot({ path: 'final-ui-persistence-check.png', fullPage: true });
    
    console.log('🔍 UI persistence check completed');
    console.log('📊 Final Results:');
    console.log('  - Collection visible after creation:', pageText?.includes('UI Test Collection') ? 'YES' : 'NO');
    console.log('  - Request visible after creation:', pageText?.includes('UI Test Request') ? 'YES' : 'NO');
    console.log('  - Data persists after refresh:', 'CHECK SCREENSHOTS');
  });

  test('should check what happens when we try to save without filling required fields', async ({ page }) => {
    ...
    if (await saveBtn.count() > 0) {
      ...
      if (await confirmBtn.count() > 0) {
        ...
        if (errorText?.includes('error') || errorText?.includes('Error') || errorText?.includes('required')) {
