...
test.describe('Collection Click to Collapse Test', () => {
  ...
  test('should expand/collapse collection when clicking anywhere on the collection item', async ({ page }) => {
  ...
  test('should not trigger collapse when clicking action menu', async ({ page }) => {
    const collectionItems = page.locator('[data-testid*="collection"], .collection-item');
    const collectionCount = await collectionItems.count();
    ...
    if (collectionCount > 0) {
      ...
      const actionMenuButton = firstCollection.locator('button[aria-haspopup], [data-testid*="menu"], button:has-text("⋯"), button:has-text("⋮")').last();
  ...
  test('should show visual feedback on hover', async ({ page }) => {
    ...
    if (collectionCount > 0) {
      ...
      const cursor = await firstCollection.evaluate(el => getComputedStyle(el).cursor);
