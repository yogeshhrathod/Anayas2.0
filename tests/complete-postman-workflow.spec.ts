...
test.describe('Complete Postman-like Workflow Validation', () => {
  ...
  test('should complete full Postman-like workflow: Create → Configure → Send → Save', async ({ page }) => {
    ...
    for (const selector of saveButtons) {
      ...
      if (await saveBtn.count() > 0) {
        ...
        if (await nameInput.count() > 0) {
          ...
          // Try to confirm save
          const confirmBtn = page.locator('button:has-text("Save"), button:has-text("Confirm"), button:has-text("Create")');
