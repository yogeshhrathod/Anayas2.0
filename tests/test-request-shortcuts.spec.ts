...
test.describe('TEST_REQUEST Keyboard Shortcuts Test', () => {
  ...
  test('should test keyboard shortcuts with TEST_REQUEST', async ({ page }) => {
    ...
    if (collectionCount > 0) {
      ...
      const addRequestButton = page.locator('button:has-text("Add Request"), button:has-text("New Request")');
      if (await addRequestButton.isVisible()) {
        ...
        if (await contextMenu.isVisible()) {
          const addRequestMenuItem = contextMenu.locator('text=Add Request').first();
      ...
      const requestNameInput = page.locator('input[placeholder*="name"], input[placeholder*="Name"]').first();
    ...
    if (testRequestExists) {
      ...
      const isSelected = requestClasses?.includes('bg-primary/10') || requestClasses?.includes('border-primary/20');
      ...
      if (isSelected) {
        ...
        for (const shortcut of shortcuts) {
          console.log(`Testing ${shortcut.name} (${shortcut.key}) on TEST_REQUEST...`);
        ...
        console.log(`\n=== Results ===`);
        console.log(`Triggered ${triggeredShortcuts.length}/${shortcuts.length} shortcuts on TEST_REQUEST`);
        ...
        console.log('⚠ TEST_REQUEST not selected - visual selection may not be working');
      ...
      const allRequests = page.locator('[data-testid*="request"], .request-item');
  ...
  test('should verify TEST_REQUEST visual selection', async ({ page }) => {
    ...
    if (await testRequest.isVisible()) {
      ...
      const requestClasses = await testRequest.getAttribute('class');
      const hasSelection = requestClasses?.includes('bg-primary/10') || requestClasses?.includes('border-primary/20');
