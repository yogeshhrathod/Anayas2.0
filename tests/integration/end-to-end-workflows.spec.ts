...
test.describe('End-to-End Workflow Tests', () => {
  test('should complete full workflow: Create Environment → Collection → Request → Send → Save', async ({ electronPage }) => {
  ...
  test('should complete workflow with folder organization', async ({ electronPage }) => {
    // Create environment
    const envResult = await electronPage.evaluate(async () => {
      return await (window as any).electronAPI.env.save({
        name: 'folder-e2e-env',
    ...
    const requests = await electronPage.evaluate(({ collectionId, folderId }) => {
  ...
  test('should complete workflow with collection environments', async ({ electronPage }) => {
  ...
  test('should complete workflow: Update → Delete operations', async ({ electronPage }) => {
    // Create environment
    const envResult = await electronPage.evaluate(async () => {
      return await (window as any).electronAPI.env.save({
  ...
  test('should complete workflow with favorite operations', async ({ electronPage }) => {
    // Create collection
    const collectionResult = await electronPage.evaluate(async () => {
      return await (window as any).electronAPI.collection.save({
  ...
  test('should complete workflow: Create → Send → History → Delete History', async ({ electronPage }) => {
    // Create environment
    const envResult = await electronPage.evaluate(async () => {
      return await (window as any).electronAPI.env.save({
