...
test.describe('UI Visual Verification', () => {
  test('should visually verify UI is rendering and interactive', async ({
    electronPage,
  }) => {
    // Step 1: Verify initial page load
    await electronPage.waitForLoadState('networkidle');
    const initialScreenshot = await electronPage.screenshot({
      path: 'test-artifacts/01-initial-load.png',
      fullPage: true,
    });
    ...
    const collectionsScreenshot = await electronPage.screenshot({
      path: 'test-artifacts/02-collections-page.png',
      fullPage: true,
    });
    ...
    await electronPage.waitForTimeout(1000);
    const formScreenshot = await electronPage.screenshot({
      path: 'test-artifacts/03-form-opened.png',
      fullPage: true,
    });
    ...
    const filledScreenshot = await electronPage.screenshot({
      path: 'test-artifacts/04-input-filled.png',
      fullPage: true,
    });
    ...
    const afterSaveScreenshot = await electronPage.screenshot({
      path: 'test-artifacts/05-after-save.png',
      fullPage: true,
    });
    ...
    const collectionCard = electronPage
      .locator('h3:has-text("Visual Test Collection")')
      .first();
    ...
    const finalScreenshot = await electronPage.screenshot({
      path: 'test-artifacts/06-collection-visible.png',
      fullPage: true,
    });
  ...
  test('should verify button clicks trigger React handlers', async ({
    electronPage,
  }) => {
    ...
    const initialCount = await electronPage.evaluate(() => {
      ...
      const collectionButtons = buttons.filter(
        btn =>
  ...
  test('should verify UI state changes are visible', async ({
    electronPage,
  }) => {
    ...
    const collectionElement = electronPage
      .locator('text=State Change Test')
      .first();
