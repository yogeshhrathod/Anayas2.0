...
test.describe('Collection IPC Handlers', () => {
  test('collection:list - should return empty list initially', async ({
    electronPage,
    _testDbPath,
  }) => {
  ...
  test('collection:save - should create new collection', async ({
    electronPage,
    _testDbPath,
  }) => {
    ...
    // Verify persistence
    assertDataPersisted(
      { id: result.id, ...collectionData },
      testDbPath,
      'collections'
    );
  });

  test('collection:save - should update existing collection', async ({
    electronPage,
    _testDbPath,
  }) => {
  ...
  test('collection:delete - should delete collection', async ({
    electronPage,
    _testDbPath,
  }) => {
    // Create collection first
  ...
  test('collection:toggleFavorite - should toggle favorite status', async ({
    electronPage,
    _testDbPath,
  }) => {
    // Create collection
    ...
    const collection2 = dbContents2.collections.find(
      (c: any) => c.id === createResult.id
    );
    expect(collection2.isFavorite).toBe(0);
  });

  test('collection:addEnvironment - should add environment to collection', async ({
    electronPage,
    _testDbPath,
  }) => {
    // Create collection
  ...
  test('collection:setActiveEnvironment - should set active environment', async ({
    electronPage,
    _testDbPath,
  }) => {
    ...
    const result = await electronPage.evaluate(
      async ({ collectionId, envId }) => {
        return await window.electronAPI.collection.setActiveEnvironment(
          collectionId,
          envId
        );
      },
  ...
  test('collection:updateEnvironment - should update collection environment', async ({
    electronPage,
    _testDbPath,
  }) => {
    // Create collection with environment
    ...
    const result = await electronPage.evaluate(
      async ({ collectionId, envId }) => {
        return await window.electronAPI.collection.updateEnvironment(
          collectionId,
          envId,
          {
            name: 'Updated Env',
  ...
  test('collection:deleteEnvironment - should delete collection environment', async ({
    electronPage,
    _testDbPath,
  }) => {
    ...
    const setup = await electronPage.evaluate(async () => {
      ...
      const _env1 = await window.electronAPI.collection.addEnvironment(
        coll.id,
        {
          name: 'Env 1',
          variables: {},
      ...
      // Set env1 as active
      await window.electronAPI.collection.setActiveEnvironment(
        coll.id,
        env1.id
      );
    ...
    const result = await electronPage.evaluate(
      async ({ collectionId, envId }) => {
        return await window.electronAPI.collection.deleteEnvironment(
          collectionId,
          envId
        );
      },
  ...
  test('collection:deleteEnvironment - should handle deleting last environment', async ({
    electronPage,
    _testDbPath,
  }) => {
  ...
  test('collection:run - should run all requests in collection', async ({
    electronPage,
    _testDbPath,
  }) => {
    // Create environment
