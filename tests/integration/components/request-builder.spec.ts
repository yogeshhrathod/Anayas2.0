...
test.describe('RequestBuilder Component Integration', () => {
  ...
  test('should send request and display response', async ({ electronPage, testDbPath }) => {
    ...
    const urlInput = electronPage.locator('input[placeholder*="URL"], input[placeholder*="Enter request URL"]').first();
    ...
    const responseBody = electronPage.locator('[class*="monaco"], text="Response Body", [class*="ResponseBody"]').first();
    const responseVisible = await responseBody.isVisible({ timeout: 10000 }).catch(() => false);
    ...
    if (!responseVisible) {
      ...
      const hasResponseContent = await electronPage.evaluate(() => {
        ...
        return bodyText.includes('userId') || bodyText.includes('id') || bodyText.includes('title');
      });
  ...
  test('should handle different HTTP methods', async ({ electronPage, testDbPath }) => {
    ...
    const methodSelect = electronPage.locator('button:has-text("GET"), [role="combobox"]:has-text("GET"), [data-testid="method-select"]').first();
    ...
    // Select POST method
    const postOption = electronPage.locator('[role="option"]:has-text("POST"), [data-value="POST"]').first();
    ...
    const methodValue = await electronPage.evaluate(() => {
      const select = document.querySelector('[data-testid="method-select"], button[role="combobox"]') as HTMLElement;
  ...
  test('should display tabs for params, headers, body, auth', async ({ electronPage, testDbPath }) => {
    ...
    const paramsTab = electronPage.locator('button:has-text("Params"), [role="tab"]:has-text("Params")').first();
    await paramsTab.waitFor({ state: 'visible', timeout: 10000 });
    
    // Verify all expected tabs are present
    const headersTab = electronPage.locator('button:has-text("Headers"), [role="tab"]:has-text("Headers")').first();
    const bodyTab = electronPage.locator('button:has-text("Body"), [role="tab"]:has-text("Body")').first();
    const authTab = electronPage.locator('button:has-text("Auth"), [role="tab"]:has-text("Auth")').first();
