...
test.describe('Sidebar Component Integration', () => {
  test('should render sidebar with navigation items', async ({
    electronPage,
    testDbPath,
  }) => {
    ...
    const homeNav = electronPage.locator(
      'button:has-text("Home"), a:has-text("Home")'
    );
    const collectionsNav = electronPage.locator(
      'button:has-text("Collections"), a:has-text("Collections")'
    );
    const envNav = electronPage.locator(
      'button:has-text("Environments"), a:has-text("Environments")'
    );
  ...
  test('should navigate between pages', async ({
    ...
    // Click Collections
    const collectionsNav = electronPage
      .locator('button:has-text("Collections"), a:has-text("Collections")')
      .first();
    ...
    // Click Environments
    const envNav = electronPage
      .locator('button:has-text("Environments"), a:has-text("Environments")')
      .first();
    await envNav.click();
    await electronPage.waitForLoadState('networkidle');
    await electronPage.waitForTimeout(1000);
    ...
    // Click Home
    const homeNav = electronPage
      .locator('button:has-text("Home"), a:has-text("Home")')
      .first();
    await homeNav.click();
    await electronPage.waitForLoadState('networkidle');
    await electronPage.waitForTimeout(1000);
  ...
  test('should update active page state', async ({
    electronPage,
    testDbPath,
  }) => {
    await electronPage.goto('/');
  ...
  test('should display collection hierarchy in sidebar', async ({
    electronPage,
    testDbPath,
  }) => {
    // Create collection
    ...
    const collectionVisible = await electronPage
      .locator('text=Sidebar Collection')
      .isVisible({ timeout: 5000 });
    expect(collectionVisible).toBe(true);
  });

  test('should refresh sidebar when data changes', async ({
    electronPage,
    testDbPath,
  }) => {
    await electronPage.goto('/');
    ...
    const collectionVisible = await electronPage
      .locator('text=Refresh Test Collection')
      .isVisible({ timeout: 5000 });
    expect(collectionVisible).toBe(true);
  });

  test('should handle sidebar toggle/collapse', async ({
    electronPage,
    testDbPath,
  }) => {
    await electronPage.goto('/');
    ...
    const toggleButton = electronPage
      .locator(
        'button[aria-label*="menu"], button[aria-label*="toggle"], [class*="menu-toggle"]'
      )
      .first();
