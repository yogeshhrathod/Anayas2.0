...
test.describe('Database Operations Tests', () => {
  test('should persist environment data', async ({
    electronPage,
    testDbPath,
  }) => {
    ...
    const savedEnv = dbContents.environments.find(
  ...
  test('should persist collection data', async ({
    electronPage,
    testDbPath,
  }) => {
    ...
    // Verify persistence
    const dbContents = getDatabaseContents(testDbPath);
    const savedCollection = dbContents.collections.find(
  ...
  test('should persist request data', async ({ electronPage, testDbPath }) => {
    ...
    // Verify persistence
    const dbContents = getDatabaseContents(testDbPath);
    const savedRequest = dbContents.requests.find(
  ...
  test('should maintain collection-request relationships', async ({
    electronPage,
    testDbPath,
  }) => {
    // Create collection
    ...
    // Verify relationship
    const dbContents = getDatabaseContents(testDbPath);
    const request = dbContents.requests.find(
      (r: any) => r.id === requestResult.id
    );
    ...
    const deletedRequest = dbContentsAfter.requests.find(
      (r: any) => r.id === requestResult.id
    );
    expect(deletedRequest).toBeUndefined();
  });

  test('should maintain folder-request relationships', async ({
    electronPage,
    testDbPath,
  }) => {
    // Create collection
  ...
  test('should persist request history', async ({
    electronPage,
    testDbPath,
  }) => {
  ...
  test('should handle data integrity on updates', async ({
    electronPage,
    testDbPath,
  }) => {
    // Create environment
    ...
    const dbContents = getDatabaseContents(testDbPath);
    const updatedEnv = dbContents.environments.find(
      (e: any) => e.id === envResult.id
    );
