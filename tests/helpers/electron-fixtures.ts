...
async function createIPCHandlers(testDbPath: string) {
  ...
  return {
    env: {
      ...
      getCurrent: async () => {
        const database = db.getDatabase();
        const defaultEnv = database.environments.find(
          (e: any) => e.isDefault === 1
        );
      ...
      test: async (env: any) => {
        try {
          ...
          if (baseUrl) {
            const isReachable =
              await apiService.apiService.testConnection(baseUrl);
            return {
              success: isReachable,
              message: isReachable
    ...
    collection: {
      ...
      setActiveEnvironment: async (
        ...
        if (environmentId !== null && environmentId !== undefined) {
          if (
            ...
            !collection.environments.find((e: any) => e.id === environmentId)
          ) {
        ...
        db.saveDatabase();
        const updatedCollection = database.collections.find(
          (c: any) => c.id === collectionId
        );
      ...
      updateEnvironment: async (
        ...
        const env = collection.environments.find(
          (e: any) => e.id === environmentId
        );
        if (!env) {
          throw new Error('Environment not found');
        }
      ...
      run: async (collectionId: number) => {
        try {
          ...
          const allRequests = database.requests.filter(
          ...
          const globalEnv =
            database.environments.find((e: any) => e.isDefault === 1) ||
            database.environments[0];
          if (!globalEnv) {
          ...
          for (let i = 0; i < sortedRequests.length; i++) {
            ...
            try {
              const resolvedUrl = variableResolver.variableResolver.resolve(
                request.url,
                variableContext
              );
              const resolvedHeaders =
                variableResolver.variableResolver.resolveObject(
                  request.headers || {},
                  variableContext
                );
              let resolvedBody =
                typeof request.body === 'string'
                  ? variableResolver.variableResolver.resolve(
                      request.body,
                      variableContext
                    )
                  : request.body;
              ...
              const resolvedQueryParams = (request.queryParams || []).map(
                (param: any) => ({
                  ...param,
                  value: variableResolver.variableResolver.resolve(
                    param.value,
                    variableContext
                  ),
                })
              ...
              switch (request.method) {
                case 'GET':
                  result = await apiService.apiService.getJson(
                    finalUrl,
                    resolvedHeaders
                  );
                  break;
                case 'POST':
                  result = await apiService.apiService.postJson(
                    finalUrl,
                    resolvedBody,
                    resolvedHeaders
                  );
                  break;
                case 'PUT':
                  result = await apiService.apiService.putJson(
                    finalUrl,
                    resolvedBody,
                    resolvedHeaders
                  );
                  break;
                case 'PATCH':
                  result = await apiService.apiService.patchJson(
                    finalUrl,
                    resolvedBody,
                    resolvedHeaders
                  );
                  break;
                case 'DELETE':
                  result = await apiService.apiService.deleteJson(
                    finalUrl,
                    resolvedHeaders
                  );
                  break;
                case 'HEAD':
                  result = await apiService.apiService.headJson(
                    finalUrl,
                    resolvedHeaders
                  );
                  break;
                case 'OPTIONS':
                  result = await apiService.apiService.optionsJson(
                    finalUrl,
                    resolvedHeaders
                  );
                  break;
          ...
          return {
            ...
            summary: {
              ...
              passed: results.filter(
                (r: any) => r.success && r.status && r.status < 400
              ).length,
      ...
      send: async (options: any) => {
        try {
          ...
          if (options.environmentId) {
            globalEnv = database.environments.find(
              (e: any) => e.id === options.environmentId
            );
          }
          if (!globalEnv) {
            globalEnv =
          ...
          if (options.collectionId) {
            const collection = database.collections.find(
              (c: any) => c.id === options.collectionId
            );
            if (
              collection &&
              collection.environments &&
          ...
          const resolvedQueryParams = (options.queryParams || []).map(
            (param: any) => ({
              ...param,
              value: variableResolver.variableResolver.resolve(
                param.value,
    ...
    folder: {
      list: async (collectionId?: number) => {
        ...
        return database.folders.filter(
          (f: any) => !collectionId || f.collectionId === collectionId
        );
      },
    ...
    curl: {
      parse: async (command: string) => {
        try {
          const parseCurlCommand =
            require('../../electron/lib/curl-parser').parseCurlCommand;
      ...
      generate: async (request: any) => {
        try {
          const generateCurlCommand =
            require('../../electron/lib/curl-generator').generateCurlCommand;
      ...
      importBulk: async (commands: string[]) => {
        try {
          const parseCurlCommands =
            require('../../electron/lib/curl-parser').parseCurlCommands;
...
async function setupMockElectronAPI(page: Page, testDbPath: string) {
  ...
  await page.exposeFunction(
    'electronAPI_env_getCurrent',
    handlers.env.getCurrent
  );
  await page.exposeFunction(
    'electronAPI_env_setCurrent',
    handlers.env.setCurrent
  );
  ...
  await page.exposeFunction(
    'electronAPI_collection_list',
    handlers.collection.list
  );
  await page.exposeFunction(
    'electronAPI_collection_save',
    handlers.collection.save
  );
  await page.exposeFunction(
    'electronAPI_collection_delete',
    handlers.collection.delete
  );
  await page.exposeFunction(
    'electronAPI_collection_toggleFavorite',
    handlers.collection.toggleFavorite
  );
  await page.exposeFunction(
    'electronAPI_collection_setActiveEnvironment',
    handlers.collection.setActiveEnvironment
  );
  await page.exposeFunction(
    'electronAPI_collection_addEnvironment',
    handlers.collection.addEnvironment
  );
  await page.exposeFunction(
    'electronAPI_collection_updateEnvironment',
    handlers.collection.updateEnvironment
  );
  await page.exposeFunction(
    'electronAPI_collection_deleteEnvironment',
    handlers.collection.deleteEnvironment
  );
  await page.exposeFunction(
    'electronAPI_collection_run',
    handlers.collection.run
  );
  ...
  await page.exposeFunction(
    'electronAPI_request_saveAfter',
    handlers.request.saveAfter
  );
  await page.exposeFunction(
    'electronAPI_request_delete',
    handlers.request.delete
  );
  await page.exposeFunction('electronAPI_request_send', handlers.request.send);
  await page.exposeFunction(
    'electronAPI_request_history',
    handlers.request.history
  );
  await page.exposeFunction(
    'electronAPI_request_deleteHistory',
    handlers.request.deleteHistory
  );
  ...
  await page.exposeFunction(
    'electronAPI_settings_getAll',
    handlers.settings.getAll
  );
  await page.exposeFunction(
    'electronAPI_settings_reset',
    handlers.settings.reset
  );

  await page.exposeFunction(
    'electronAPI_unsavedRequest_save',
    handlers.unsavedRequest.save
  );
  await page.exposeFunction(
    'electronAPI_unsavedRequest_getAll',
    handlers.unsavedRequest.getAll
  );
  await page.exposeFunction(
    'electronAPI_unsavedRequest_delete',
    handlers.unsavedRequest.delete
  );
  await page.exposeFunction(
    'electronAPI_unsavedRequest_clear',
    handlers.unsavedRequest.clear
  );
  await page.exposeFunction(
    'electronAPI_unsavedRequest_promote',
    handlers.unsavedRequest.promote
  );
  ...
  await page.exposeFunction(
    'electronAPI_preset_delete',
    handlers.preset.delete
  );

  await page.exposeFunction('electronAPI_curl_parse', handlers.curl.parse);
  await page.exposeFunction(
    'electronAPI_curl_generate',
    handlers.curl.generate
  );
  await page.exposeFunction(
    'electronAPI_curl_importBulk',
    handlers.curl.importBulk
  );

  await page.exposeFunction('electronAPI_file_select', handlers.file.select);
  await page.exposeFunction(
    'electronAPI_file_selectDirectory',
    handlers.file.selectDirectory
  );
  ...
  await page.exposeFunction(
    'electronAPI_app_getVersion',
    handlers.app.getVersion
  );
  await page.exposeFunction('electronAPI_app_getPath', handlers.app.getPath);

  await page.exposeFunction(
    'electronAPI_window_minimize',
    handlers.window.minimize
  );
  await page.exposeFunction(
    'electronAPI_window_maximize',
    handlers.window.maximize
  );
  await page.exposeFunction('electronAPI_window_close', handlers.window.close);
  await page.exposeFunction(
    'electronAPI_window_isMaximized',
    handlers.window.isMaximized
  );

  await page.exposeFunction(
    'electronAPI_notification_show',
    handlers.notification.show
  );
    ...
    (window as any).electronAPI = {
      env: {
        ...
        setCurrent: (id: number) =>
          (window as any).electronAPI_env_setCurrent(id),
        test: (env: any) => (window as any).electronAPI_env_test(env),
        import: (filePath: string) =>
          (window as any).electronAPI_env_import(filePath),
      },
      collection: {
        list: () => (window as any).electronAPI_collection_list(),
        save: async (collection: any) => {
          const result = await (window as any).electronAPI_collection_save(
            collection
          );
        ...
        delete: async (id: number) => {
          const result = await (window as any).electronAPI_collection_delete(
            id
          );
          collectionUpdatedListeners.forEach(cb => cb());
        ...
        toggleFavorite: async (id: number) => {
          const result = await (
            window as any
          ).electronAPI_collection_toggleFavorite(id);
          collectionUpdatedListeners.forEach(cb => cb());
          return result;
        },
        setActiveEnvironment: (
          ...
          (window as any).electronAPI_collection_setActiveEnvironment(
            collectionId,
            environmentId
          ),
        addEnvironment: async (collectionId: number, environment: any) => {
          const result = await (
            window as any
          ).electronAPI_collection_addEnvironment(collectionId, environment);
          collectionUpdatedListeners.forEach(cb => cb());
          return result;
        },
        updateEnvironment: async (
        ...
        run: (collectionId: number) =>
          (window as any).electronAPI_collection_run(collectionId),
      ...
      request: {
        ...
        save: async (request: any) => {
          const result = await (window as any).electronAPI_request_save(
            request
          );
          requestUpdatedListeners.forEach(cb => cb());
          return result;
        ...
        saveAfter: async (request: any, afterRequestId: number) => {
          const result = await (window as any).electronAPI_request_saveAfter(
            request,
            afterRequestId
          );
          requestUpdatedListeners.forEach(cb => cb());
        ...
        send: (options: any) =>
          (window as any).electronAPI_request_send(options),
        history: (limit?: number) =>
          (window as any).electronAPI_request_history(limit),
        deleteHistory: (id: number) =>
          (window as any).electronAPI_request_deleteHistory(id),
        onUpdated: (callback: () => void) => {
      ...
      folder: {
        list: (collectionId?: number) =>
          (window as any).electronAPI_folder_list(collectionId),
        save: async (folder: any) => {
      ...
      settings: {
        ...
        set: (key: string, value: any) =>
          (window as any).electronAPI_settings_set(key, value),
        getAll: () => (window as any).electronAPI_settings_getAll(),
        reset: () => (window as any).electronAPI_settings_reset(),
      },
      unsavedRequest: {
        save: (request: any) =>
          (window as any).electronAPI_unsavedRequest_save(request),
        getAll: () => (window as any).electronAPI_unsavedRequest_getAll(),
        delete: (id: number) =>
          (window as any).electronAPI_unsavedRequest_delete(id),
        clear: () => (window as any).electronAPI_unsavedRequest_clear(),
        promote: (id: number, data: any) =>
          (window as any).electronAPI_unsavedRequest_promote(id, data),
      },
      preset: {
        list: (requestId?: number) =>
          (window as any).electronAPI_preset_list(requestId),
      ...
      curl: {
        parse: (command: string) =>
          (window as any).electronAPI_curl_parse(command),
        generate: (request: any) =>
          (window as any).electronAPI_curl_generate(request),
        importBulk: (commands: string[]) =>
          (window as any).electronAPI_curl_importBulk(commands),
      },
      file: {
        select: (filters?: any) =>
          (window as any).electronAPI_file_select(filters),
        selectDirectory: () =>
          (window as any).electronAPI_file_selectDirectory(),
        save: (defaultPath: string, content: string) =>
          (window as any).electronAPI_file_save(defaultPath, content),
        read: (filePath: string) =>
          (window as any).electronAPI_file_read(filePath),
        write: (filePath: string, content: string) =>
          (window as any).electronAPI_file_write(filePath, content),
      },
      app: {
        getVersion: () => (window as any).electronAPI_app_getVersion(),
        getPath: (name: string) =>
          (window as any).electronAPI_app_getPath(name),
      },
      window: {
      ...
      notification: {
        show: (options: any) =>
          (window as any).electronAPI_notification_show(options),
      },
    };
  });
}
