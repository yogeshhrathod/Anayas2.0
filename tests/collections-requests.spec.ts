  ...
  test.describe('Collections Management', () => {
    ...
    test('should create a new collection', async ({ page }) => {
      ...
      const createButton = page
        .locator(
          'button:has-text("New Collection"), button:has-text("Create Collection"), button:has-text("Add Collection")'
        )
        .first();
      if (await createButton.isVisible()) {
        ...
        const nameInput = page
          .locator(
            'input[placeholder*="name" i], input[placeholder*="collection" i]'
          )
          .first();
    ...
    test('should display collections hierarchy in sidebar', async ({
      page,
    }) => {
      ...
      const collectionItems = page.locator(
        '[data-testid*="collection"], .collection-item, .folder-item'
      );
    ...
    test('should create a new folder within collection', async ({ page }) => {
      ...
      const createFolderButton = page
        .locator(
          'button:has-text("New Folder"), button:has-text("Create Folder"), button:has-text("Add Folder")'
        )
        .first();
      if (await createFolderButton.isVisible()) {
        ...
        const nameInput = page
          .locator(
            'input[placeholder*="name" i], input[placeholder*="folder" i]'
          )
          .first();
        if (await nameInput.isVisible()) {
    ...
    test('should edit collection name', async ({ page }) => {
      ...
      const collectionItem = page
        .locator('[data-testid*="collection"], .collection-item')
        .first();
  ...
  test.describe('Request Management', () => {
    test('should create a new API request', async ({ page }) => {
      ...
      const createRequestButton = page
        .locator(
          'button:has-text("New Request"), button:has-text("Create Request"), button:has-text("Add Request")'
        )
        .first();
    ...
    test('should fill request details', async ({ page }) => {
      ...
      if (await urlInput.isVisible()) {
        ...
        const methodSelect = page
          .locator('select, button:has-text("GET"), button:has-text("POST")')
          .first();
        ...
        const headersTab = page
          .locator('button:has-text("Headers"), [data-testid*="header"]')
          .first();
        ...
        const bodyTab = page
          .locator('button:has-text("Body"), [data-testid*="body"]')
          .first();
    ...
    test('should send API request', async ({ page }) => {
      ...
      if (await urlInput.isVisible()) {
        ...
        const sendButton = page
          .locator(
            'button:has-text("Send"), button:has-text("Execute"), button:has-text("Run")'
          )
          .first();
    ...
    test('should save request to collection', async ({ page }) => {
      ...
      if (await urlInput.isVisible()) {
        ...
        if (await saveButton.isVisible()) {
          ...
          const nameInput = page
            .locator(
              'input[placeholder*="name" i], input[placeholder*="request" i]'
            )
            .first();
          if (await nameInput.isVisible()) {
            ...
            if (await collectionSelect.isVisible()) {
              ...
              await page
                .click('text=Default Collection, text=My Collection')
                .first();
            }
            ...
            if (await confirmButton.isVisible()) {
              ...
              await expect(
                page.locator('text=Get Users Request')
              ).toBeVisible();
            }
          }
        }
    ...
    test('should edit existing request', async ({ page }) => {
      ...
      const requestItem = page
        .locator('[data-testid*="request"], .request-item')
        .first();
    ...
    test('should test different HTTP methods', async ({ page }) => {
      ...
      if (await urlInput.isVisible()) {
        ...
        const methodButton = page
          .locator('button:has-text("GET"), button:has-text("POST"), select')
          .first();
        ...
        if (await methodButton.isVisible()) {
          ...
          if (await bodyTab.isVisible()) {
            ...
            const bodyTextarea = page
              .locator('textarea, .monaco-editor')
              .first();
            if (await bodyTextarea.isVisible()) {
              await bodyTextarea.fill(
        ...
        if (await authTab.isVisible()) {
          ...
          if (await authTypeSelect.isVisible()) {
            ...
            const tokenInput = page
              .locator(
                'input[placeholder*="token" i], input[placeholder*="key" i]'
              )
              .first();
  ...
  test.describe('Request History', () => {
    test('should view request history', async ({ page }) => {
      ...
      const historyItems = page.locator(
        '[data-testid*="history"], .history-item'
      );
    ...
    test('should clear request history', async ({ page }) => {
      ...
      const clearButton = page
        .locator('button:has-text("Clear"), button:has-text("Clear History")')
        .first();
      if (await clearButton.isVisible()) {
        ...
        const confirmButton = page
          .locator(
            'button:has-text("Clear"), button:has-text("Confirm"), button:has-text("Yes")'
          )
          .first();
        if (await confirmButton.isVisible()) {
          await confirmButton.click();
