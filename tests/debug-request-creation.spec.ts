...
test.describe('Debug Request Creation - Postman-like Workflow', () => {
  ...
  test('should create a new request like Postman - step by step debug', async ({
    page,
  }) => {
    ...
    if (urlInputCount > 0) {
      ...
      await urlInput
        .first()
        .fill('https://jsonplaceholder.typicode.com/posts/1');
      ...
      const sendBtn = page.locator(
        'button:has-text("Send"), button:has-text("Execute")'
      );
      ...
      if (sendBtnCount > 0) {
        ...
        // Check for response
        const responseSection = page.locator(
          '[data-testid*="response"], .response-panel, text=200'
        );
      ...
      const saveBtn = page.locator(
        'button:has-text("Save"), button:has-text("Save Request")'
      );
      ...
      if (saveBtnCount > 0) {
        ...
        const saveDialog = page.locator(
          'input[placeholder*="name" i], text=Save Request'
        );
        ...
        if (dialogCount > 0) {
          ...
          if ((await nameInput.count()) > 0) {
            ...
            const collectionSelect = page.locator(
              'select, button:has-text("Select Collection")'
            );
            if ((await collectionSelect.count()) > 0) {
              ...
              const collectionOption = page
                .locator('text=JSONPlaceholder API, text=Default Collection')
                .first();
            ...
            if ((await confirmBtn.count()) > 0) {
              ...
              if ((await savedRequest.count()) > 0) {
                ...
                await page.screenshot({
                  path: 'debug-request-not-in-sidebar.png',
                });
              }
            }
          }
  ...
  test('should navigate to Collections and create request from there', async ({
    page,
  }) => {
    ...
    console.log(
      'Found',
      buttonCount,
      'New Request buttons on Collections page'
    );
    ...
    if (buttonCount > 0) {
      ...
      const urlInputCount = await urlInput.count();
      console.log(
        'After clicking New Request, found',
        urlInputCount,
        'URL input fields'
      );
      ...
      if (urlInputCount > 0) {
        ...
        console.log(
          '❌ Request builder not visible after clicking New Request'
        );
        await page.screenshot({
          path: 'debug-collections-new-request-failed.png',
        });
      }
    } else {
      console.log('❌ No New Request button found on Collections page');
      await page.screenshot({
        path: 'debug-collections-no-new-request-button.png',
      });
    }
  });
