...
test.describe('Collection Description Removal Test', () => {
  ...
  test('should not display collection descriptions in hierarchy', async ({
    page,
  }) => {
    ...
    if (collectionCount > 0) {
      ...
      for (let i = 0; i < collectionCount; i++) {
        ...
        const collectionName = collectionItem.locator(
          'span.text-sm.font-medium'
        );
        await expect(collectionName).toBeVisible();

        // Verify no description text is shown
        const descriptionText = collectionItem.locator(
  ...
  test('should maintain clean collection layout without descriptions', async ({
    page,
  }) => {
    const collectionItems = page.locator(
      '[data-testid*="collection"], .collection-item'
    ...
    if (collectionCount > 0) {
      ...
      const collectionName = firstCollection.locator(
        'span.text-sm.font-medium'
      );
      await expect(collectionName).toBeVisible();
