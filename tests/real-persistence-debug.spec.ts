...
test.describe('Real Data Persistence Debug', () => {
  ...
  test('should debug what actually gets saved to the database', async ({
    page,
  }) => {
    ...
    const existingCollections = page.locator(
      '.collection-item, text=Collection'
    );
            ...
            if ((await nameInput.count()) > 0) {
              ...
              if ((await confirmBtn.count()) > 0) {
                ...
                if ((await savedRequest.count()) > 0) {
                  ...
                  console.log(
                    '❌ Request does NOT appear in sidebar after save'
                  );
    ...
    await page.screenshot({
      path: 'final-persistence-check.png',
      fullPage: true,
    });
    ...
    console.log(
      '  - Collections created:',
      (await page.locator('text=Debug Test Collection').count()) > 0
        ? 'YES'
        : 'NO'
    );
    console.log(
      '  - Requests created:',
      (await page.locator('text=Debug Test Request').count()) > 0 ? 'YES' : 'NO'
    );
    console.log('  - Data persists after refresh:', 'CHECK SCREENSHOTS');
  });

  test('should check what happens when we try to save without collection', async ({
    page,
  }) => {
      ...
      if ((await nameInput.count()) > 0) {
        ...
        if ((await confirmBtn.count()) > 0) {
          ...
          const errorMsg = page.locator(
            'text=error, text=Error, text=required, text=Required'
          );
