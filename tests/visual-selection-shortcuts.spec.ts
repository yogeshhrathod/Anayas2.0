...
test.describe('Visual Selection and Keyboard Shortcuts Test', () => {
  ...
  test('should show visual selection when collection is clicked', async ({
    page,
  }) => {
    ...
    if (collectionCount > 0) {
      ...
      const selectedCollection = firstCollection.locator(
  ...
  test('should show visual selection when request is clicked', async ({
    page,
  }) => {
    const collectionItems = page.locator(
      '[data-testid*="collection"], .collection-item'
  ...
  test('should trigger keyboard shortcuts when item is selected', async ({
    page,
  }) => {
    const collectionItems = page.locator(
      '[data-testid*="collection"], .collection-item'
  ...
  test('should trigger delete shortcut and actually delete item', async ({
    page,
  }) => {
    const collectionItems = page.locator(
      '[data-testid*="collection"], .collection-item'
  ...
  test('should not show selection when no item is clicked', async ({
    page,
  }) => {
    const collectionItems = page.locator(
      '[data-testid*="collection"], .collection-item'
  ...
  test('should clear selection when clicking elsewhere', async ({ page }) => {
    ...
    if (collectionCount > 1) {
      ...
      const secondCollectionClasses =
        await secondCollection.getAttribute('class');
      expect(secondCollectionClasses).toContain('bg-primary/10');
    }
  });

  test('should show different shortcuts for different item types', async ({
    page,
  }) => {
    const collectionItems = page.locator(
      '[data-testid*="collection"], .collection-item'
