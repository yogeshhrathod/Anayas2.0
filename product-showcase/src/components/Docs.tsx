...
export function Docs() {
  const [activeTab, setActiveTab] = useState<'api' | 'cli' | 'architecture'>('api');
       ...
       <div className="container mx-auto px-4 md:px-6">
           <div className="flex flex-col md:flex-row gap-12">
               ...
               <div className="w-full md:w-64 shrink-0 space-y-8">
                   <div>
                       <h3 className="text-primary font-mono text-xs uppercase tracking-widest mb-4">Core Modules</h3>
                       <nav className="flex flex-col space-y-1">
                           {['Introduction', 'Installation', 'Authentication', 'Requests', 'Responses'].map((item) => (
                               <a key={item} href="#" className="font-mono text-sm text-gray-400 hover:text-white transition-colors py-1 block">
                                   {item}
                               </a>
                           ))}
                       </nav>
                   ...
                   <div>
                       <h3 className="text-primary font-mono text-xs uppercase tracking-widest mb-4">Advanced</h3>
                       <nav className="flex flex-col space-y-1">
                           {['Testing', 'Environment Vars', 'CI/CD Integration', 'Plugins'].map((item) => (
                               <a key={item} href="#" className="font-mono text-sm text-gray-400 hover:text-white transition-colors py-1 block">
                                   {item}
                               </a>
                           ))}
               ...
               <div className="flex-1">
                   <div className="mb-12 border-b border-white/10 pb-8">
                       <h1 className="text-4xl md:text-6xl font-display font-bold uppercase tracking-tight mb-4">System Manual</h1>
                       <p className="text-xl text-gray-500 font-mono">v0.0.1-alpha // PREVIEW BUILD</p>
                   </div>
                       ...
                       <div className="p-6 min-h-[300px] font-mono text-sm">
                           {activeTab === 'api' && (
                               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                   <div className="text-primary mb-4">GET /v1/system/status</div>
...
{`{
  ...
  "modules": [
    ...
    "encryption: secured"
                           ...
                           {activeTab === 'cli' && (
                               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                   <div className="text-primary mb-4">$ luna run --test ./spec/*</div>
                                   <div className="text-gray-400 mb-2">// Output stream</div>
                                   <div className="space-y-1 text-gray-300">
                                       <div>[INIT] Loading configuration... OK</div>
                                       <div>[TEST] benchmarks/latency.spec.ts... <span className="text-green-500">PASS (12ms)</span></div>
                                       <div>[TEST] integration/auth_flow.spec.ts... <span className="text-green-500">PASS (45ms)</span></div>
                               ...
                               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                                   <div className="grid grid-cols-2 gap-4">
                                       <div className="border border-white/10 p-4">
                                           ...
                                           <div className="text-gray-500 text-xs">Rust-based IO layer for zero-latency request handling.</div>
                                       </div>
                                       <div className="border border-white/10 p-4">
                                           <Shield className="w-6 h-6 text-primary mb-2" />
                                           <div className="font-bold mb-1">Vault</div>
                                           <div className="text-gray-500 text-xs">AES-256 encrypted local storage for sensitive tokens.</div>
                                       </div>
                                   </div>
                               </motion.div>
                           )}
                   ...
                   <div className="prose prose-invert max-w-none">
                       ...
                       <p className="text-gray-400 font-mono leading-relaxed mb-8">
                           Luna is designed to be drop-in ready. It parses your existing OpenAPI/Swagger definitions and instantly generates a testable collection. 
                           No configuration required. Just point to your schema and start sending requests.
                       </p>
