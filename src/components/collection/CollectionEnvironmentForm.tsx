...
export interface CollectionEnvironmentFormProps {
  ...
  onSave: (data: { name: string; variables: Record<string, string> }) => Promise<void>;
  onCancel: () => void;
  isLoading?: boolean;
  showActions?: boolean;
}
...
export const CollectionEnvironmentForm = forwardRef<CollectionEnvironmentFormRef, CollectionEnvironmentFormProps>(({
  environment,
  onSave,
  onCancel,
  isLoading = false,
  ...
  const handleVariableChange = useCallback((variables: Record<string, string>) => {
