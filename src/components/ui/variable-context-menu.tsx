...
export const VariableContextMenu = forwardRef<HTMLDivElement, VariableContextMenuProps>(({
  ...
  const handleEdit = () => {
    console.log('[VariableContextMenu] handleEdit called for variable:', variableName);
    ...
    console.log('[VariableContextMenu] Available environments count:', environments.length);
    console.log('[VariableContextMenu] Available collections count:', collections.length);
    ...
    if (variable?.scope === 'collection' && selectedRequest?.collectionId) {
      ...
      if (collection) {
        ...
        console.log('[VariableContextMenu] Editing collection variable, opening collection:', collection.id);
        setCollectionToEditId(collection.id!);
        setCurrentPage('collections');
        return;
      } else {
        console.log('[VariableContextMenu] Collection not found for ID:', selectedRequest.collectionId);
      }
    }
    
    // For global variables, find the environment that contains this variable
    console.log('[VariableContextMenu] Searching for global variable in environments...');
    let environmentToOpen: { id: number; name: string } | null = null;
    
    // Check if variable exists in any environment
    console.log('[VariableContextMenu] Checking', environments.length, 'environments for variable:', variableName);
    for (const env of environments) {
      console.log('[VariableContextMenu] Checking environment:', env.name, 'ID:', env.id, 'Variables:', Object.keys(env.variables || {}));
      if (env.variables && variableName in env.variables) {
        environmentToOpen = { id: env.id!, name: env.name };
        console.log('[VariableContextMenu] Found variable in environment:', env.name, 'ID:', env.id);
        break;
      }
    }
    ...
    if (environmentToOpen) {
      ...
      console.log('[VariableContextMenu] Setting environmentToEditId to:', environmentToOpen.id, 'for variable:', variableName);
      ...
      console.log('[VariableContextMenu] Variable not found in any environment, navigating to environments');
      setCurrentPage('environments');
    }
  };
