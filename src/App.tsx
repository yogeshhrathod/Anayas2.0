    ...
    try {
      ...
      showError(
        ...
        error instanceof Error
          ? error.message
          : 'Failed to clear unsaved requests'
      );
    }
  };
  ...
  useShortcuts({
    ...
    'edit-item': (_: KeyboardEvent, context: ContextState) => {
      if (context.selectedItem.type === 'request') {
        ...
        setSelectedRequest(context.selectedItem.data as Request | null);
        setCurrentPage('home');
    ...
    'duplicate-item': async (_: KeyboardEvent, context: ContextState) => {
      if (context.selectedItem.type === 'request') {
        try {
          const request = context.selectedItem.data as Request;
          const duplicate = {
        ...
        try {
          const collection = context.selectedItem.data as Collection;
          const duplicate = {
      ...
      <div className="flex h-screen flex-col bg-background">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="flex flex-1 flex-col">
            ...
            {/* Page Content */}
            <div
              className={`flex-1 overflow-auto ${currentPage === 'home' ? 'p-0' : 'p-4'}`}
            >
              {renderPage()}
            </div>
          </div>
