...
export function useCollectionOperations() {
  ...
  const { setCollections: setGlobalCollections, triggerSidebarRefresh } =
    useStore();
  ...
  const updateCollection = useCallback(
    async (id: number, data: CollectionFormData) => {
      try {
        const result = await window.electronAPI.collection.save({
          id,
          name: data.name,
  ...
  const exportCollections = useCallback(async () => {
    try {
      ...
      showSuccess('Export successful', {
        description: 'Collections have been exported successfully',
      });
    } catch (error: any) {
      ...
      input.onchange = async e => {
        ...
        if (Array.isArray(importedCollections)) {
          ...
          showSuccess('Import successful', {
            description: `${importedCollections.length} collections imported successfully`,
          });
        } else {
          showError(
            'Invalid file format',
            'Please select a valid collections export file'
          );
        }
      };
