...
export function useCollectionEnvironmentOperations(
  ...
  const createEnvironment = useCallback(async (data: CollectionEnvironmentFormData) => {
    setIsLoading(true);
    try {
      const result = await window.electronAPI.collection.addEnvironment(collectionId, data);
      if (result?.success) {
        showSuccess('Environment created', { description: `${data.name} has been created` });
  ...
  const updateEnvironment = useCallback(async (
    ...
    data: CollectionEnvironmentFormData
  ) => {
    setIsLoading(true);
    try {
      ...
      if (result?.success) {
        showSuccess('Environment updated', { description: `${data.name} has been updated` });
        // Reload collection if callback provided
        if (onUpdate && result.collection) {
          onUpdate(result.collection);
        }
  ...
  const deleteEnvironment = useCallback(async (environmentId: number) => {
    setDeletingId(environmentId);
    try {
      const result = await window.electronAPI.collection.deleteEnvironment(collectionId, environmentId);
      if (result?.success) {
        showSuccess('Environment deleted', { description: 'Environment has been deleted' });
        // Reload collection if callback provided
        if (onUpdate && result.collection) {
          onUpdate(result.collection);
        }
  ...
  const setActiveEnvironment = useCallback(async (environmentId: number | null) => {
    try {
      const result = await window.electronAPI.collection.setActiveEnvironment(collectionId, environmentId);
      if (result?.success) {
        showSuccess('Environment activated', { description: 'Active environment has been updated' });
        // Reload collection if callback provided
        if (onUpdate && result.collection) {
          onUpdate(result.collection);
        }
