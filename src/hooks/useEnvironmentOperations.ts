...
export function useEnvironmentOperations() {
  ...
  const createEnvironment = useCallback(
    async (data: EnvironmentFormData) => {
      try {
        const envData = {
          name: data.name,
  ...
  const updateEnvironment = useCallback(
    async (id: number, data: EnvironmentFormData) => {
      try {
        const envData = {
          id,
          name: data.name,
  ...
  const exportEnvironments = useCallback(async () => {
    try {
      ...
      showSuccess('Export successful', {
        description: 'Environments have been exported successfully',
      });
    } catch (error: unknown) {
      showError(
        'Failed to export environments',
        error instanceof Error ? error.message : 'Failed to export environments'
      );
    }
  ...
  const importEnvironments = useCallback(async () => {
    try {
      ...
      input.onchange = async e => {
        ...
        if (Array.isArray(importedEnvironments)) {
          ...
          showSuccess('Import successful', {
            description: `${importedEnvironments.length} environments imported successfully`,
          });
        } else {
          showError(
            'Invalid file format',
            'Please select a valid environments export file'
          );
        }
      };
      ...
      showError(
        ...
        error instanceof Error ? error.message : 'Failed to import environments'
      );
    }
