...
export function useCollectionDragDrop(config: DragDropConfig) {
  ...
  const canDrop = useCallback(
    (draggedItem: DragItem, targetItem: DragItem): boolean => {
      ...
      if (draggedItem.type === 'request') {
        ...
        if (targetItem.type === 'request') {
          ...
          return (
            draggedItem.collectionId === targetItem.collectionId &&
  ...
  const handleDragOver = useCallback(
    (e: React.DragEvent, item: DragItem, position?: DropPosition) => {
      e.preventDefault();
      ...
      if (
        !position &&
        state.draggedItem.type === item.type &&
        e.currentTarget
      ) {
  ...
  const handleDrop = useCallback(
    async (
      ...
      if (
        !position &&
        state.draggedItem.type === targetItem.type &&
        e.currentTarget
      ) {
        try {
          const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
      ...
      try {
        if (state.draggedItem.type === 'request') {
          if (
            ...
            if (config.onMoveRequest) {
              ...
              if (targetItem.type === 'collection') {
                ...
                if (config.resolveFolderCollectionId) {
                  targetCollectionId =
                    config.resolveFolderCollectionId(targetItem.id as number) ||
                    0;
                } else {
              ...
              const targetFolderId =
                targetItem.type === 'folder'
                  ? (targetItem.id as number)
                  : undefined;

              await config.onMoveRequest(
            ...
            // Reorder operation
            if (
              config.onReorderRequest &&
              (dropPosition === 'above' || dropPosition === 'below')
            ) {
              await config.onReorderRequest(
                state.draggedItem.id,
                targetItem.id,
                dropPosition
              );
          ...
          if (targetItem.type === 'collection') {
            ...
            if (config.onMoveFolder) {
              await config.onMoveFolder(
                state.draggedItem.id as number,
                targetItem.id as number
              );
            }
            ...
            // Reorder operation
            if (
              config.onReorderFolder &&
              (dropPosition === 'above' || dropPosition === 'below')
            ) {
              await config.onReorderFolder(
                state.draggedItem.id as number,
                targetItem.id as number,
                dropPosition
              );
